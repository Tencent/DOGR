...
from evaluation.utils import compute_grd_f1_qwen, compute_text_sim, get_raw_string
from evaluation.mplug_evaluator import doc_evaluate
import ast
from PIL import Image


#F1 score threshold should be set
iou_threshold=0.1
...
def convert_to_format(answer, bbox):
    # Extract the relevant parts from the input
    answer_text = answer.replace("\n", " ").strip().strip(".").strip().lower()
    bbox_values = bbox.replace("[", "").replace("]", "").replace(" ","")

    # Format the output
    formatted_output = f"<ocr>{answer_text}</ocr><bbox>{bbox_values}</bbox>"
...
def preprocess_short_answer(texts, types=None,extract=False):
    ...
    new_texts = []
    for t in texts:
        if "Bbox:" in t and "Answer:" in t:
            t = t.split("Answer:")[-1]
            t0 = t.split("Bbox:")[0]
            t1 = t.split("Bbox:")[-1]
            t = convert_to_format(t0, t1)
            # print(t)
        new_texts.append(t)
    texts = new_texts
...
def preprocess_short_answer_qwen(texts, types=None,extract=False):
    for i,t in enumerate(texts):
        if t==None:
            texts[i]=''
    texts = [" ".join(t.split()) for t in texts]
    ...
    return texts
# def evalute_opened_qa(preds, gts,wh, metric_names=["BLEU1", "BLEU4", "RougeL", "Meteor"]):
def evalute_opened_qa(preds, gts,wh, metric_names=["BLEU1", "BLEU4", "RougeL"],qs=None,iou_threshold=iou_threshold):
    ...
    F1all, F1loc,F1rcg = compute_grd_f1_qwen(preds, gts,wh,iou_threshold=iou_threshold,qs=qs)
    F1all_detail, F1loc_detail,F1rcg_detail = compute_grd_f1_qwen(preds, gts,wh,iou_threshold=iou_threshold, verbose=True,qs=qs)
...
def evaluate_short_answer(preds, gts,wh,types, metric_names=["RelaxedAccuracy"]):
...
def evaluate_short_grounding_answer(preds, gts,wh, metric_names=["RelaxedAccuracy"],iou_threshold=iou_threshold):
    metric2score = {}
    preds = preprocess_short_answer_qwen(preds)
    for metric_name in metric_names:
        pred_texts = preprocess_short_answer(preds)
        gt_texts = [preprocess_short_answer(gt) for gt in gts]
    ...
    # grounding accuracy
    F1all, F1loc,F1rcg = compute_grd_f1_qwen(preds, gts,wh,iou_threshold=iou_threshold)
    F1all_detail, F1loc_detail,F1rcg_detail = compute_grd_f1_qwen(preds, gts,wh,iou_threshold=iou_threshold, verbose=True)
    metric2score["F1all"] = (F1all, F1all_detail)
    metric2score["F1loc"] = (F1loc, F1loc_detail)
    metric2score["F1rcg"] = (F1rcg, F1rcg_detail)
...
def evaluate_long_reasoning_qa(preds, gts,wh,types, box_list=None, metric_names=["ExactAccuracy"],qs=None,iou_threshold=iou_threshold):
    ...
    F1all, F1loc,F1rcg = compute_grd_f1_qwen(preds, gts,wh,iou_threshold=iou_threshold, box_list=box_list,qs=qs)
    F1all_detail, F1loc_detail,F1rcg_detail = compute_grd_f1_qwen(preds, gts,wh,iou_threshold=iou_threshold,box_list=box_list, verbose=True,qs=qs)
    metric2score["F1all"] = (F1all, F1all_detail)
    metric2score["F1loc"] = (F1loc, F1loc_detail)
    metric2score["F1rcg"] = (F1rcg, F1rcg_detail)
...
def parse_predict_results(pred_results):
    ...
    data_dict = {
        "POSTER": {"Rt": {"preds": [], "gts": [],'wh':[]}, "Ga": {"preds": [], "gts": [],'wh':[]},"GRa": {"preds": [], "gts": [],'wh':[]}, "Gr": {"preds": [], "gts": [], "bboxes": [],'wh':[]},"GRr": {"preds": [], "gts": [], "bboxes": [],'qs':[],'wh':[]}, "Go": {"preds": [], "gts": [],'wh':[]},"GRo": {"preds": [], "gts": [],'qs':[],'wh':[]}}, 
        "CHART": {"Rt": {"preds": [], "gts": [],"types":[],'wh':[]}, "Ga": {"preds": [], "gts": [],'wh':[]},"GRa": {"preds": [], "gts": [],'wh':[]}, "Gr": {"preds": [], "gts": [], "bboxes": [],"types":[],'wh':[]},"GRr": {"preds": [], "gts": [], "bboxes": [],"types":[],'qs':[],'wh':[]}, "Go": {"preds": [], "gts": [],"types":[],'wh':[]},"GRo": {"preds": [], "gts": [],'qs':[],'wh':[]}}, 
        "PDF": {"Rt": {"preds": [], "gts": [],'wh':[]}, "Ga": {"preds": [], "gts": [],'wh':[]},"GRa": {"preds": [], "gts": [],'wh':[]}, "Gr": {"preds": [], "gts": [],'wh':[]},"GRr": {"preds": [], "gts": [],'qs':[],'wh':[]}, "Go": {"preds": [], "gts": [],'wh':[]},"GRo": {"preds": [], "gts": [],'qs':[],'wh':[]}}, 
        }
    ...
    for item in pred_results:
        ...
        img_path = item['image']
        img_path = "/group/40079/uasonchen/projects/qwen2vl/source_data/doc_downstream/"+img_path

        # image = Image.open(img_path)
        # width, height = image.size
        width, height = item['wh']
        

        data_dict[data_name][task_name]["wh"].append((width, height))
        # print(item)
...
if __name__ == "__main__":
    ...
    score_dict = {
        "POSTER": {"Rt": None, "Ga": None, "Gr": None, "Go": None,"GRo": None},
    ...
    for data_type, type_items in data_dict.items():
        ...
        for task_type, task_items in type_items.items():
            ...
            if task_type == "Rt":
                score_dict[data_type][task_type] = evaluate_short_answer(task_items["preds"], task_items["gts"],task_items["wh"],types=task_items["types"] if "types" in task_items else None, metric_names=["RelaxedAccuracy"])
            elif task_type == "Gr" or task_type == "GRr":
                score_dict[data_type][task_type] = evaluate_long_reasoning_qa(task_items["preds"], task_items["gts"],task_items["wh"],types=task_items["types"] if "types" in task_items else None, metric_names=["RelaxedAccuracy"], box_list=task_items["bboxes"] if "bboxes" in task_items else None,qs=task_items["qs"] if "qs" in task_items else None,iou_threshold=iou_threshold)
            elif task_type == "Ga" or task_type == "GRa":
                score_dict[data_type][task_type] = evaluate_short_grounding_answer(task_items["preds"], task_items["gts"],task_items["wh"], metric_names=["RelaxedAccuracy"],iou_threshold=iou_threshold)
            else:
                score_dict[data_type][task_type] = evalute_opened_qa(task_items["preds"], task_items["gts"],task_items["wh"],qs=task_items["qs"] if "qs" in task_items else None,iou_threshold=iou_threshold)
