...
def bi_contain_match(target:str, prediction:str):
    def has_word(sentence, word):
        pattern = r"\b" + re.escape(word) + r"\b"
        match = re.search(pattern, sentence)
        if match:
            ...
            return False
    # print(prediction, target, float(has_word(prediction, target)))
    return float(has_word(prediction, target) or has_word(target, prediction))
...
def metric_calculate(
    ...
    score = (100.0 * total) / len(targets)  if len(targets)>0 else 0
    return score, scores
...
def doc_evaluate(
    ...
    elif metric == 'BiContainAccuracy':
        score, scores = metric_calculate(targets, predictions, metric_fn=bi_contain_match, normalize_fn=remove_special_chars_and_lower)
    elif metric == 'CIDEr':
