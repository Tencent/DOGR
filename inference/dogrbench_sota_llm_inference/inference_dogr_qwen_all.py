...
class CustomImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if "Gr" in data_type:
            text_prompt += "You should prepare the key text of your reasoning and corresponding bbox coordinate in JSON format, like```json\n[{\"bbox_2d\": [x1,y1,x2,y2], \"text\": \"\"}\n]```. The text should be exactly the content in bbox_2d. After the json output, give a reasoning using the text in your json output. At the end, you should give a short and simple final answer following 'Answer:'. " #  x1, y1, x2, y2 in the normalized coordinates in [0,999].
        elif "Ga" in data_type:
            text_prompt +=  "Answer the question with a simple answer following ' Answer: ' and corresponding bbox coordinate in [x1, y1, x2, y2] format follwing ' Bbox: '. [x1, y1, x2, y2] is the bounding box of the answer."
        elif "Go" in data_type:
            text_prompt += "You should prepare the key text of your reasoning and corresponding bbox coordinate in JSON format, like```json\n[{\"bbox_2d\": [x1,y1,x2,y2], \"text\": \"\"}\n]```. The text should be exactly the content in bbox_2d. After the json output, give a reasoning using the text in your json output." #  x1, y1, x2, y2 in the normalized coordinates in [0,999]
        else:
...
if __name__ == "__main__":
    ...
    processor = AutoProcessor.from_pretrained("Qwen/Qwen2.5-VL-7B-Instruct", max_pixels=1280*28*28)
    ...
    for items, text_prompts, imgs, videos in tqdm.tqdm(dataloader):
        for item, text_prompt, img, video in zip(items, text_prompts, imgs, videos):
            ...
            if "Ga" in item["task_name"] or "GRa" in item["task_name"] or "Rt" in item["task_name"]:
                continue
            # try:
            messages = [
                {
