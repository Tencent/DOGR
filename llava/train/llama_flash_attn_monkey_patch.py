...
def forward(
    ...
    if key_padding_mask is None:
        ...
        cu_q_lens = torch.arange(0, (bsz + 1) * q_len, step=q_len, dtype=torch.int32, device=f'npu:{qkv.device}' if isinstance(qkv.device, int) else qkv.device)
        max_s = q_len
...
def replace_llama_attn_with_flash_attn():
    cuda_major, cuda_minor = torch.npu.get_device_capability()
    if cuda_major < 8:
