...
class ModelArguments:
    ...
    mm_vision_downsample_ratio: Optional[float] = field(default=None)
...
class DataArguments:
    ...
    best_resolution_select_mode: Optional[str] = field(default="ceiling")
...
def preprocess_qwen(sources, tokenizer: transformers.PreTrainedTokenizer, has_image: bool = False, max_len=2048, system_message: str = "You are a helpful assistant.") -> Dict:
    ...
    for i, source in enumerate(sources):
        ...
        if source[0]["from"] not in roles or roles[source[0]["from"]] != roles["human"]:
            source = source[1:]
...
class LazySupervisedDataset(Dataset):
    def __init__(self, data_path: str, tokenizer: transformers.PreTrainedTokenizer, data_args: DataArguments):
        ...
        if "{" in data_path and "}" in data_path:
            ...
            if data_path.endswith(".json"):
                ...
                with open(data_path, "r") as file:
                    cur_data_dict = [json.loads(line) for line in file]
    ...
    def process_image(self, image_file, overwrite_image_aspect_ratio=None):
        ...
        if image_aspect_ratio == "highres":
            ...
            image = process_anyres_image(image, self.data_args.image_processor, self.data_args.image_grid_pinpoints, self.data_args.best_resolution_select_mode)
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        for attempt_idx in range(num_base_retries):
            try:
                ...
                if "image" in self.list_data_dict[i]:
                    print(f"[Try #{attempt_idx}] Failed to fetch sample {i}, {self.list_data_dict[i]['image']}, Exception:", e)
                else:
        ...
        for attempt_idx in range(num_base_retries):
            try:
                ...
                # no need to sleep
                if "image" in self.list_data_dict[next_index]:
                    print(f"[Try other #{attempt_idx}] Failed to fetch sample {next_index}, {self.list_data_dict[next_index]['image']}, Exception:", e)
                else:
                    print(f"[Try other #{attempt_idx}] Failed to fetch sample {next_index}. Exception:", e)
                pass
        
        # 如果连续两个样本都坏了，就会报错。所以这里修改一下
        # try:
        #     sample = self._get_item(i)
        #     return sample
        # except Exception as e:
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        # 有的样本中有image,但image=None,这种情况删掉image的kv
        if "image" in sources and sources["image"] is None:
            sources.pop("image")
        if isinstance(i, int):
            sources = [sources]
        ...
        elif "video" in sources[0] and sources[0]["video"] is not None:
...
def get_model(model_args, training_args, bnb_model_from_pretrained_args):
    ...
    if model_args.mm_vision_downsample_ratio is not None:
        overwrite_config["mm_vision_downsample_ratio"] = model_args.mm_vision_downsample_ratio
    if overwrite_config:
    ...
    if model_args.model_class_name is not None:
        ...
        if "mixtral" in model_args.model_name_or_path.lower():
            ...
            or ("yi" in model_args.model_name_or_path.lower() and "yinanzhou" not in model_args.model_name_or_path.lower())
...
def train(attn_implementation=None):
    ...
    if model_args.vision_tower is not None:
        ...
        vision_tower.to(dtype=torch.bfloat16 if training_args.bf16 else torch.float16, device=f'npu:{training_args.device}' if isinstance(training_args.device, int) else training_args.device)
        ...
        if data_args.image_grid_pinpoints is not None:
            if isinstance(data_args.image_grid_pinpoints, str) and "x" in data_args.image_grid_pinpoints:
            ...
            elif isinstance(data_args.image_grid_pinpoints, str) and data_args.image_grid_pinpoints.isdigit():
                try:
                    patch_size = data_args.image_processor.size[0]
                except Exception as e:
                    patch_size = data_args.image_processor.size["shortest_edge"]
                ...
                max_tile_num = int(data_args.image_grid_pinpoints)
                grid_pinpoints = [(i, j) for i in range(1, max_tile_num+1) for j in range(1, max_tile_num+1) if i * j <= max_tile_num]
                data_args.image_grid_pinpoints = [[dim * patch_size for dim in pair] for pair in grid_pinpoints]
        ...
        if training_args.bits in [4, 8]:
            model.get_model().mm_projector.to(dtype=compute_dtype, device=f'npu:{training_args.device}' if isinstance(training_args.device, int) else training_args.device)

        model.config.mm_use_im_start_end = data_args.mm_use_im_start_end = model_args.mm_use_im_start_end
        model.config.mm_projector_lr = training_args.mm_projector_lr
        model.config.mm_vision_tower_lr = training_args.mm_vision_tower_lr
        model.config.best_resolution_select_mode = data_args.best_resolution_select_mode
