...
def train(attn_implementation=None):
    ...
    if training_args.lora_enable:
        ...
        if training_args.bits == 16:
            if training_args.bf16:
                model.to(f'npu:{torch.bfloat16}' if isinstance(torch.bfloat16, int) else torch.bfloat16)
            if training_args.fp16:
                model.to(f'npu:{torch.float16}' if isinstance(torch.float16, int) else torch.float16)
    ...
    if model_args.vision_tower is not None:
        ...
        vision_tower.to(f'npu:{torch.bfloat16 if training_args.bf16 else torch.float16}' if isinstance(torch.bfloat16 if training_args.bf16 else torch.float16, int) else torch.bfloat16 if training_args.bf16 else torch.float16, device=f'npu:{training_args.device}' if isinstance(training_args.device, int) else training_args.device)
        ...
        if training_args.bits in [4, 8]:
            model.get_model().mm_projector.to(f'npu:{compute_dtype}' if isinstance(compute_dtype, int) else compute_dtype, device=f'npu:{training_args.device}' if isinstance(training_args.device, int) else training_args.device)
        ...
        if ref_model is not None:
            ...
            ref_vision_tower.to(f'npu:{torch.bfloat16 if training_args.bf16 else torch.float16}' if isinstance(torch.bfloat16 if training_args.bf16 else torch.float16, int) else torch.bfloat16 if training_args.bf16 else torch.float16, device=f'npu:{training_args.device}' if isinstance(training_args.device, int) else training_args.device)
    ...
    if training_args.bits in [4, 8]:
        ...
        for name, module in model.named_modules():
            if isinstance(module, LoraLayer):
                if training_args.bf16:
                    module = module.to(f'npu:{torch.bfloat16}' if isinstance(torch.bfloat16, int) else torch.bfloat16)
            if "norm" in name:
                module = module.to(f'npu:{torch.float32}' if isinstance(torch.float32, int) else torch.float32)
