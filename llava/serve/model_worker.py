...
class ModelWorker:
    ...
    def generate_stream(self, params):
        ...
        if images is not None and len(images) > 0 and self.is_multimodal:
            if len(images) > 0:
                ...
                if type(images) is list:
                    images = [image.to(f'npu:{self.model.device}' if isinstance(self.model.device, int) else self.model.device, dtype=torch.float16) for image in images]
                else:
                    images = images.to(f'npu:{self.model.device}' if isinstance(self.model.device, int) else self.model.device, dtype=torch.float16)
        ...
        input_ids = tokenizer_image_token(prompt, tokenizer, IMAGE_TOKEN_INDEX, return_tensors="pt").unsqueeze(0).npu()
        keywords = [stop_str]
    ...
    def generate_stream_gate(self, params):
        ...
        except torch.npu.CudaError as e:
            print("Caught torch.npu.CudaError:", e)
            ret = {
                "text": server_error_msg,
                "error_code": 1,
            }
