...
class LlamaRMSNorm(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        hidden_states = hidden_states.to(f'npu:{torch.float32}' if isinstance(torch.float32, int) else torch.float32)
        variance = hidden_states.pow(2).mean(-1, keepdim=True)
        hidden_states = hidden_states * torch.rsqrt(variance + self.variance_epsilon)
        return self.weight * hidden_states.to(f'npu:{input_dtype}' if isinstance(input_dtype, int) else input_dtype)
...
class LlamaRotaryEmbedding(nn.Module):
    def __init__(self, dim, max_position_embeddings=2048, base=10000, device=None, scaling_factor=1.0):
        ...
        self.base = base
        inv_freq = 1.0 / (self.base ** (torch.arange(0, self.dim, 2, dtype=torch.int64).float().to(f'npu:{device}' if isinstance(device, int) else device) / self.dim))
        self.register_buffer("inv_freq", inv_freq, persistent=False)
        # For BC we register cos and sin cached
        self.max_seq_len_cached = max_position_embeddings
        t = torch.arange(self.max_seq_len_cached, device=f'npu:{device}' if isinstance(device, int) else device, dtype=torch.int64).type_as(self.inv_freq)
        t = t / self.scaling_factor
        freqs = torch.outer(t, self.inv_freq)
        # Different from paper, but it uses a different permutation in order to obtain the same calculation
        emb = torch.cat((freqs, freqs), dim=-1)
        self.register_buffer("_cos_cached", emb.cos().to(f'npu:{torch.get_default_dtype()}' if isinstance(torch.get_default_dtype(), int) else torch.get_default_dtype()), persistent=False)
        self.register_buffer("_sin_cached", emb.sin().to(f'npu:{torch.get_default_dtype()}' if isinstance(torch.get_default_dtype(), int) else torch.get_default_dtype()), persistent=False)
    ...
    def forward(self, x, position_ids, seq_len=None):
        ...
        with torch.autocast(device_type=device_type, enabled=False):
            ...
            cos = emb.cos()
            sin = emb.sin()
        return cos.to(f'npu:{x.dtype}' if isinstance(x.dtype, int) else x.dtype), sin.to(f'npu:{x.dtype}' if isinstance(x.dtype, int) else x.dtype)
...
class LlamaDynamicNTKScalingRotaryEmbedding(LlamaRotaryEmbedding):
    ...
    def forward(self, x, position_ids, seq_len=None):
        ...
        if seq_len > self.max_position_embeddings:
            ...
            inv_freq = 1.0 / (base ** (torch.arange(0, self.dim, 2, dtype=torch.int64).float().to(f'npu:{x.device}' if isinstance(x.device, int) else x.device) / self.dim))
...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        attn_weights = nn.functional.softmax(attn_weights, dim=-1, dtype=torch.float32).to(f'npu:{query_states.dtype}' if isinstance(query_states.dtype, int) else query_states.dtype)
...
class LlamaRingFlashAttention2(LlamaAttention):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if input_dtype == torch.float32:
            ...
            query_states = query_states.to(f'npu:{target_dtype}' if isinstance(target_dtype, int) else target_dtype)
            key_states = key_states.to(f'npu:{target_dtype}' if isinstance(target_dtype, int) else target_dtype)
            value_states = value_states.to(f'npu:{target_dtype}' if isinstance(target_dtype, int) else target_dtype)
    ...
    def _upad_input(self, query_layer, key_layer, value_layer, attention_mask, query_length):
        ...
        if query_length == kv_seq_len:
            ...
            cu_seqlens_q = torch.arange(batch_size + 1, dtype=torch.int32, device=f'npu:{query_layer.device}' if isinstance(query_layer.device, int) else query_layer.device)  # There is a memcpy here, that is very bad.
...
class LlamaPreTrainedModel(PreTrainedModel):
    ...
    def _setup_cache(self, cache_cls, max_batch_size, max_cache_len: Optional[int] = None):
        ...
        if max_cache_len > self.model.causal_mask.shape[-1] or self.device != self.model.causal_mask.device:
            causal_mask = torch.full((max_cache_len, max_cache_len), fill_value=True, device=f'npu:{self.device}' if isinstance(self.device, int) else self.device, dtype=torch.bool)
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        if cache_position is None:
            ...
            cache_position = torch.arange(past_seen_tokens, past_seen_tokens + inputs_embeds.shape[1], device=f'npu:{inputs_embeds.device}' if isinstance(inputs_embeds.device, int) else inputs_embeds.device)
    ...
    def _update_causal_mask(self, attention_mask, input_tensor):
        ...
        causal_mask = self.causal_mask[None, None, :, :].repeat(batch_size, 1, 1, 1).to(f'npu:{dtype}' if isinstance(dtype, int) else dtype) * min_dtype

        causal_mask = causal_mask.to(f'npu:{dtype}' if isinstance(dtype, int) else dtype, device=f'npu:{device}' if isinstance(device, int) else device)
        if attention_mask is not None and attention_mask.dim() == 2:
            mask_length = attention_mask.shape[-1]
            padding_mask = causal_mask[..., :mask_length].eq(0.0) * attention_mask[:, None, None, :].eq(0.0)
            causal_mask[..., :mask_length] = causal_mask[..., :mask_length].masked_fill(padding_mask, min_dtype)

        if self.config._attn_implementation == "sdpa" and attention_mask is not None and attention_mask.device.type == "npu":
...
class LlamaForCausalLM(LlamaPreTrainedModel):
    ...
    def prepare_inputs_for_generation(self, input_ids, past_key_values=None, attention_mask=None, inputs_embeds=None, **kwargs):
        ...
        cache_position = torch.arange(past_length, past_length + input_length, device=f'npu:{input_ids.device}' if isinstance(input_ids.device, int) else input_ids.device)
    ...
    def _reorder_cache(past_key_values, beam_idx):
        ...
        for layer_past in past_key_values:
            reordered_past += (tuple(past_state.index_select(0, beam_idx.to(f'npu:{past_state.device}' if isinstance(past_state.device, int) else past_state.device)) for past_state in layer_past),)
    ...
    def forward(
        ...
        if self.config.pad_token_id is None:
            ...
            if input_ids is not None:
                ...
                sequence_lengths = sequence_lengths.to(f'npu:{logits.device}' if isinstance(logits.device, int) else logits.device)
            else:
                sequence_lengths = -1

        pooled_logits = logits[torch.arange(batch_size, device=f'npu:{logits.device}' if isinstance(logits.device, int) else logits.device), sequence_lengths]

        loss = None
        if labels is not None:
            labels = labels.to(f'npu:{logits.device}' if isinstance(logits.device, int) else logits.device)
    ...
    def forward(
        ...
        if start_positions is not None and end_positions is not None:
            ...
            if len(start_positions.size()) > 1:
                start_positions = start_positions.squeeze(-1).to(f'npu:{start_logits.device}' if isinstance(start_logits.device, int) else start_logits.device)
            if len(end_positions.size()) > 1:
                end_positions = end_positions.squeeze(-1).to(f'npu:{end_logits.device}' if isinstance(end_logits.device, int) else end_logits.device)
