...
class BertSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if self.position_embedding_type == "relative_key" or self.position_embedding_type == "relative_key_query":
            ...
            position_ids_l = torch.arange(seq_length, dtype=torch.long, device=f'npu:{hidden_states.device}' if isinstance(hidden_states.device, int) else hidden_states.device).view(-1, 1)
            position_ids_r = torch.arange(seq_length, dtype=torch.long, device=f'npu:{hidden_states.device}' if isinstance(hidden_states.device, int) else hidden_states.device).view(1, -1)
            distance = position_ids_l - position_ids_r
            positional_embedding = self.distance_embedding(distance + self.max_position_embeddings - 1)
            positional_embedding = positional_embedding.to(f'npu:{query_layer.dtype}' if isinstance(query_layer.dtype, int) else query_layer.dtype)  # fp16 compatibility
...
class BertModel(BertPreTrainedModel):
    ...
    def get_extended_attention_mask(
        ...
        if attention_mask.dim() == 3:
            ...
            if is_decoder:
                ...
                seq_ids = torch.arange(seq_length, device=f'npu:{device}' if isinstance(device, int) else device)
                causal_mask = seq_ids[None, None, :].repeat(batch_size, seq_length, 1) <= seq_ids[None, :, None]

                # add a prefix ones mask to the causal mask
                # causal and attention masks must have same type with pytorch version < 1.3
                causal_mask = causal_mask.to(f'npu:{attention_mask.dtype}' if isinstance(attention_mask.dtype, int) else attention_mask.dtype)
                        ...
                        causal_mask = torch.cat(
                            [
                                torch.zeros(
                                    ...
                                    device=f'npu:{device}' if isinstance(device, int) else device,
        ...
        extended_attention_mask = extended_attention_mask.to(f'npu:{self.dtype}' if isinstance(self.dtype, int) else self.dtype)  # fp16 compatibility
    ...
    def forward(
        ...
        if attention_mask is None:
            attention_mask = torch.ones(((batch_size, seq_length + past_key_values_length)), device=f'npu:{device}' if isinstance(device, int) else device)
        ...
        if encoder_hidden_states is not None:
            ...
            if type(encoder_attention_mask) == list:
                ...
                encoder_attention_mask = torch.ones(encoder_hidden_shape, device=f'npu:{device}' if isinstance(device, int) else device)
...
class Qformer(nn.Module):
    ...
    def forward(self, image_features, *args, **kwargs):
        ...
        image_atts = torch.ones(x.size()[:-1], dtype=torch.long).to(f'npu:{x.device}' if isinstance(x.device, int) else x.device)
