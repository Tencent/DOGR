...
class MaskedDrop(nn.Module):
    ...
    def random_masking(self, x, len_keep):
        ...
        noise = torch.rand(N, L, device=f'npu:{x.device}' if isinstance(x.device, int) else x.device)  # noise in [0, 1]
        ...
        mask = torch.ones([N, L], device=f'npu:{x.device}' if isinstance(x.device, int) else x.device)
        mask[:, :len_keep] = 0
