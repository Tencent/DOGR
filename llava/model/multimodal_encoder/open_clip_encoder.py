...
class OpenCLIPVisionTower(nn.Module):
    ...
    def load_model(self, device_map="auto"):
        ...
        vision_tower, _, image_processor = open_clip.create_model_and_transforms(model_name=self.model_name, pretrained=self.pretrained, precision="fp32", device="npu")
    ...
    def forward_visual(self, x, output_hidden_states=False):
        if hasattr(self.vision_tower, "trunk") and hasattr(self.vision_tower.trunk, "_intermediate_layers"):
            ...
            def forward_openclip(self, x: torch.Tensor):
                ...
                x = torch.cat(
                    [_expand_token(self.class_embedding, x.shape[0]).to(f'npu:{x.dtype}' if isinstance(x.dtype, int) else x.dtype), x],
                    dim=1,
                )
    ...
    def forward(self, images):
        if type(images) is list:
            image_features = []
            for image in images:
                image_forward_out = self.forward_visual(image.to(f'npu:{self.dtype}' if isinstance(self.dtype, int) else self.dtype).unsqueeze(0), output_hidden_states=True)
            ...
            image_forward_outs = self.forward_visual(images.to(f'npu:{self.dtype}' if isinstance(self.dtype, int) else self.dtype), output_hidden_states=True)
