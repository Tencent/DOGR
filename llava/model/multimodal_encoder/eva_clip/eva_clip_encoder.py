...
class EvaClipVisionTower(nn.Module):
    ...
    def forward(self, images):
        if type(images) is list:
            image_features = []
            for image in images:
                image_feature = self.vision_tower(image.to(f'npu:{self.device}' if isinstance(self.device, int) else self.device, dtype=self.dtype).unsqueeze(0)).to(f'npu:{image.dtype}' if isinstance(image.dtype, int) else image.dtype)
                image_features.append(image_feature)
        else:
            image_features = self.vision_tower(images.to(f'npu:{self.device}' if isinstance(self.device, int) else self.device, dtype=self.dtype)).to(f'npu:{images.dtype}' if isinstance(images.dtype, int) else images.dtype)
