...
class ImageBindWrapper(nn.Module):
    ...
    def forward(self, x):
        if type(x) == dict:
            ...
            inputs = {ModalityType.VISION: x.to(f'npu:{self.dtype}' if isinstance(self.dtype, int) else self.dtype)}
            embeddings = self.vision_tower(inputs)
    ...
    def dummy_feature(self):
        return torch.zeros(1, 1024, device=f'npu:{self.device}' if isinstance(self.device, int) else self.device, dtype=self.dtype)
