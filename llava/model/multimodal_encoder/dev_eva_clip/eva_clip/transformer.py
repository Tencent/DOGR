...
class LayerNorm(nn.LayerNorm):
    ...
    def forward(self, x: torch.Tensor):
        ...
        return x.to(f'npu:{orig_type}' if isinstance(orig_type, int) else orig_type)
...
class ResidualAttentionBlock(nn.Module):
    ...
    def attention(self, x: torch.Tensor, attn_mask: Optional[torch.Tensor] = None):
        attn_mask = attn_mask.to(f'npu:{x.dtype}' if isinstance(x.dtype, int) else x.dtype) if attn_mask is not None else None
        if self.xattn:
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, return_all_features: bool = False):
        ...
        x = torch.cat([self.class_embedding.to(f'npu:{x.dtype}' if isinstance(x.dtype, int) else x.dtype) + torch.zeros(x.shape[0], 1, x.shape[-1], dtype=x.dtype, device=f'npu:{x.device}' if isinstance(x.device, int) else x.device), x], dim=1)  # shape = [*, grid ** 2 + 1, width]
        x = x + self.positional_embedding.to(f'npu:{x.dtype}' if isinstance(x.dtype, int) else x.dtype)
