...
def create_model(
    ...
    if pretrained and pretrained.lower() == "openai":
        ...
        if "fp16" in precision or "bf16" in precision:
            ...
            model = model.to(f'npu:{torch.bfloat16}' if isinstance(torch.bfloat16, int) else torch.bfloat16) if "bf16" in precision else model.to(f'npu:{torch.float16}' if isinstance(torch.float16, int) else torch.float16)
