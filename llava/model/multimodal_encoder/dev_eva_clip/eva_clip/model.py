...
class CLIP(nn.Module):
    ...
    def encode_text(self, text, normalize: bool = False):
        ...
        x = self.token_embedding(text).to(f'npu:{cast_dtype}' if isinstance(cast_dtype, int) else cast_dtype)  # [batch_size, n_ctx, d_model]

        x = x + self.positional_embedding.to(f'npu:{cast_dtype}' if isinstance(cast_dtype, int) else cast_dtype)
...
def convert_weights_to_lp(model: nn.Module, dtype=torch.float16):
    ...
    def _convert_weights(l):
        ...
        if isinstance(l, (nn.Conv1d, nn.Conv2d, nn.Linear)):
            l.weight.data = l.weight.data.to(f'npu:{dtype}' if isinstance(dtype, int) else dtype)
            if l.bias is not None:
                l.bias.data = l.bias.data.to(f'npu:{dtype}' if isinstance(dtype, int) else dtype)

        if isinstance(l, (nn.MultiheadAttention, Attention)):
            for attr in [*[f"{s}_proj_weight" for s in ["in", "q", "k", "v"]], "in_proj_bias", "bias_k", "bias_v"]:
                tensor = getattr(l, attr, None)
                if tensor is not None:
                    tensor.data = tensor.data.to(f'npu:{dtype}' if isinstance(dtype, int) else dtype)

        if isinstance(l, nn.Parameter):
            l.data = l.data.to(f'npu:{dtype}' if isinstance(dtype, int) else dtype)

        for name in ["text_projection", "proj"]:
            if hasattr(l, name) and isinstance(l, nn.Parameter):
                attr = getattr(l, name, None)
                if attr is not None:
                    attr.data = attr.data.to(f'npu:{dtype}' if isinstance(dtype, int) else dtype)
...
def trace_model(model, batch_size=256, device=torch.device("cpu")):
    ...
    example_images = torch.ones((batch_size, 3, image_size, image_size), device=f'npu:{device}' if isinstance(device, int) else device)
    example_text = torch.zeros((batch_size, model.context_length), dtype=torch.int, device=f'npu:{device}' if isinstance(device, int) else device)
