...
class HFTextEncoder(nn.Module):
    ...
    def mask(self, input_ids, vocab_size, device, targets=None, masked_indices=None, probability_matrix=None):
        ...
        random_words = torch.randint(vocab_size, input_ids.shape, dtype=torch.long).to(f'npu:{device}' if isinstance(device, int) else device)
    ...
    def forward_mlm(self, input_ids, image_embeds, mlm_probability=0.25):
        ...
        image_atts = torch.ones(image_embeds.size()[:-1], dtype=torch.long).to(f'npu:{input_ids.device}' if isinstance(input_ids.device, int) else input_ids.device)
