...
class EvaViTWrapper(nn.Module):
    ...
    def forward(self, images):
        if type(images) is list:
            image_features = []
            for image in images:
                image_features = self.vision_tower.forward_features(image.to(f'npu:{self.dtype}' if isinstance(self.dtype, int) else self.dtype), return_all_features=True)
                image_features = self.feature_select(image_features).to(f'npu:{self.dtype}' if isinstance(self.dtype, int) else self.dtype)
                image_features.append(image_features)
        else:
            image_features = self.vision_tower.forward_features(images.to(f'npu:{self.dtype}' if isinstance(self.dtype, int) else self.dtype), return_all_features=True)
            image_features = self.feature_select(image_features).to(f'npu:{self.dtype}' if isinstance(self.dtype, int) else self.dtype)
