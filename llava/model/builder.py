    ...
    if "llava" in model_name.lower() or is_multimodal:
        ...
        if "lora" in model_name.lower() and model_base is not None:
            ...
            if model.lm_head.weight.shape[0] != token_num:
                model.lm_head.weight = torch.nn.Parameter(torch.empty(token_num, tokem_dim, device=f'npu:{model.device}' if isinstance(model.device, int) else model.device, dtype=model.dtype))
                model.model.embed_tokens.weight = torch.nn.Parameter(torch.empty(token_num, tokem_dim, device=f'npu:{model.device}' if isinstance(model.device, int) else model.device, dtype=model.dtype))
            ...
            mm_projector_weights = {k: v.to(f'npu:{torch.float16}' if isinstance(torch.float16, int) else torch.float16) for k, v in mm_projector_weights.items()}
