...
def randn_tensor(
    ...
    if isinstance(generator, list):
        shape = (1,) + shape[1:]
        latents = [torch.randn(shape, generator=generator[i], device=f'npu:{rand_device}' if isinstance(rand_device, int) else rand_device, dtype=dtype, layout=layout) for i in range(batch_size)]
        latents = torch.cat(latents, dim=0).to(f'npu:{device}' if isinstance(device, int) else device)
    else:
        latents = torch.randn(shape, generator=generator, device=f'npu:{rand_device}' if isinstance(rand_device, int) else rand_device, dtype=dtype, layout=layout).to(f'npu:{device}' if isinstance(device, int) else device)
