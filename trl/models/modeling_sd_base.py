...
def _get_variance(self, timestep, prev_timestep):
    alpha_prod_t = torch.gather(self.alphas_cumprod, 0, timestep.cpu()).to(f'npu:{timestep.device}' if isinstance(timestep.device, int) else timestep.device)
    alpha_prod_t_prev = torch.where(
        prev_timestep.cpu() >= 0,
        self.alphas_cumprod.gather(0, prev_timestep.cpu()),
        self.final_alpha_cumprod,
    ).to(f'npu:{timestep.device}' if isinstance(timestep.device, int) else timestep.device)
...
def scheduler_step(
    ...
    alpha_prod_t = _left_broadcast(alpha_prod_t, sample.shape).to(f'npu:{sample.device}' if isinstance(sample.device, int) else sample.device)
    alpha_prod_t_prev = _left_broadcast(alpha_prod_t_prev, sample.shape).to(f'npu:{sample.device}' if isinstance(sample.device, int) else sample.device)
    ...
    std_dev_t = _left_broadcast(std_dev_t, sample.shape).to(f'npu:{sample.device}' if isinstance(sample.device, int) else sample.device)
        ...
        if self.use_lora:
            ...
            for param in self.sd_pipeline.unet.parameters():
                ...
                if param.requires_grad:
                    param.data = param.to(f'npu:{torch.float32}' if isinstance(torch.float32, int) else torch.float32)
            return self.sd_pipeline.unet
        else:
            return self.sd_pipeline.unet
