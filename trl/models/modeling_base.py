...
class PreTrainedModelWrapper(nn.Module):
    ...
    def _get_current_device(cls):
        ...
        if is_xpu_available():
            ...
            return state.local_process_index if torch_npu.npu.is_available() else "cpu"
    ...
    def add_and_load_reward_modeling_adapter(cls, pretrained_model, adapter_model_id, adapter_name="reward_model_adapter", token=None):
        ...
        for name, param in adapter_state_dict.items():
            if score_name in name:
                ...
                score_dict[key_name] = param.to(f'npu:{cls._get_current_device()}' if isinstance(cls._get_current_device(), int) else cls._get_current_device())

        num_labels, hidden_dim = score_dict["weight"].shape
        has_bias = any(["bias" in name for name in adapter_state_dict.keys()])

        score = nn.Linear(hidden_dim, num_labels, bias=has_bias).to(
            f'npu:{cls._get_current_device()}' if isinstance(cls._get_current_device(), int) else cls._get_current_device(), dtype=pretrained_model.dtype,
        )
