...
class ValueHead(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        if output.dtype != self.summary.weight.dtype:
            output = output.to(f'npu:{self.summary.weight.dtype}' if isinstance(self.summary.weight.dtype, int) else self.summary.weight.dtype)
...
class AutoModelForCausalLMWithValueHead(PreTrainedModelWrapper):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if last_hidden_state.device != self.v_head.summary.weight.device:
            last_hidden_state = last_hidden_state.to(f'npu:{self.v_head.summary.weight.device}' if isinstance(self.v_head.summary.weight.device, int) else self.v_head.summary.weight.device)
    ...
    def post_init(self, state_dict):
        ...
        if hasattr(self.pretrained_model, "hf_device_map"):
            ...
            self.v_head = self.v_head.to(f'npu:{first_device}' if isinstance(first_device, int) else first_device)

            def set_device_hook(module, input, outputs):
                new_output = ()
                for output in outputs:
                    if isinstance(output, torch.Tensor):
                        new_output += (output.to(f'npu:{first_device}' if isinstance(first_device, int) else first_device),)
                    else:
                        new_output += (output,)
                return new_output
...
class AutoModelForSeq2SeqLMWithValueHead(PreTrainedModelWrapper):
    ...
    def post_init(self, state_dict):
        ...
        if hasattr(self.pretrained_model, "hf_device_map"):
            ...
            self.v_head = self.v_head.to(f'npu:{lm_head_device}' if isinstance(lm_head_device, int) else lm_head_device)
            ...
            def set_device_hook(module, input, outputs):
                ...
                for output in outputs:
                    if isinstance(output, torch.Tensor):
                        new_output += (output.to(f'npu:{lm_head_device}' if isinstance(lm_head_device, int) else lm_head_device),)
                    else:
                        new_output += (output,)
                return new_output
