...
class TextHistory:
    ...
    def __init__(self, text, tokens, system=True):
        ...
        self.text_spans = []
        self.token_spans = []
        self.token_masks = torch.tensor([], dtype=torch.long).to(f'npu:{tokens.device}' if isinstance(tokens.device, int) else tokens.device)
        self.text = ""
        self.tokens = torch.tensor([], dtype=torch.long).to(f'npu:{tokens.device}' if isinstance(tokens.device, int) else tokens.device)
...
class TextEnvironment:
    ...
    def run(self, queries, **rewards_kwargs):
        ...
        queries_tokens = [self.tokenizer(query, return_tensors="pt").input_ids[0].to(f'npu:{self.model.pretrained_model.device}' if isinstance(self.model.pretrained_model.device, int) else self.model.pretrained_model.device) for query in queries]
    ...
    def step(self, history):
        ...
        history.append_segment(
            ...
            self.tokenizer(response + self.response_token, return_tensors="pt").input_ids[0].to(f'npu:{self.model.pretrained_model.device}' if isinstance(self.model.pretrained_model.device, int) else self.model.pretrained_model.device),
            system=True,
        )
