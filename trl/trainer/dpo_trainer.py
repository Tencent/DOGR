...
class DPOTrainer(Trainer):
    ...
    def compute_reference_log_probs(self, padded_batch: Dict) -> Dict:
        ...
        compte_ref_context_manager = torch.npu.amp.autocast if self._peft_has_been_casted_to_bf16 else nullcontext
        ...
        for k in batch:
            if k.startswith("rejected") and isinstance(batch[k], torch.Tensor):
                ...
                concatenated_batch[concatenated_key] = torch.cat(
                    ...
                    dim=0,
                ).to(f'npu:{device}' if isinstance(device, int) else device)

        if is_encoder_decoder:
            concatenated_batch["concatenated_input_ids"] = batch["prompt_input_ids"].repeat(2, 1).to(f'npu:{device}' if isinstance(device, int) else device)
            concatenated_batch["concatenated_attention_mask"] = batch["prompt_attention_mask"].repeat(2, 1).to(f'npu:{device}' if isinstance(device, int) else device)
        # import pdb; pdb.set_trace()
        # repeated_list = [
        #     batch['images'][0] * 2,
        #     batch['images'][1] * 2
    ...
    def dpo_loss(
        ...
        if self.reference_free:
            ref_logratios = torch.tensor([0], dtype=pi_logratios.dtype, device=f'npu:{pi_logratios.device}' if isinstance(pi_logratios.device, int) else pi_logratios.device)
        else:
            ref_logratios = reference_chosen_logps - reference_rejected_logps

        pi_logratios = pi_logratios.to(f'npu:{self.accelerator.device}' if isinstance(self.accelerator.device, int) else self.accelerator.device)
        ref_logratios = ref_logratios.to(f'npu:{self.accelerator.device}' if isinstance(self.accelerator.device, int) else self.accelerator.device)
        ...
        chosen_rewards = self.beta * (policy_chosen_logps.to(f'npu:{self.accelerator.device}' if isinstance(self.accelerator.device, int) else self.accelerator.device) - reference_chosen_logps.to(f'npu:{self.accelerator.device}' if isinstance(self.accelerator.device, int) else self.accelerator.device)).detach()
        rejected_rewards = self.beta * (policy_rejected_logps.to(f'npu:{self.accelerator.device}' if isinstance(self.accelerator.device, int) else self.accelerator.device) - reference_rejected_logps.to(f'npu:{self.accelerator.device}' if isinstance(self.accelerator.device, int) else self.accelerator.device)).detach()
    ...
    def concatenated_forward(self, model: nn.Module, batch: Dict[str, Union[List, torch.LongTensor]]) -> Tuple[torch.FloatTensor, torch.FloatTensor, torch.FloatTensor, torch.FloatTensor]:
        ...
        all_logits = all_logits.to(f'npu:{torch.float32}' if isinstance(torch.float32, int) else torch.float32)
    ...
    def compute_loss(
        ...
        compute_loss_context_manager = torch.npu.amp.autocast if self._peft_has_been_casted_to_bf16 else nullcontext
    ...
    def get_batch_samples(self, model, batch: Dict[str, torch.LongTensor]) -> Tuple[str, str]:
        ...
        generate_context_manager = nullcontext if not self._peft_has_been_casted_to_bf16 else torch.npu.amp.autocast
    ...
    def prediction_step(
        ...
        prediction_context_manager = torch.npu.amp.autocast if self._peft_has_been_casted_to_bf16 else nullcontext
        ...
        logits = torch.stack(logits).mean(axis=1).to(f'npu:{self.accelerator.device}' if isinstance(self.accelerator.device, int) else self.accelerator.device)
        labels = torch.zeros(logits.shape[0], device=f'npu:{self.accelerator.device}' if isinstance(self.accelerator.device, int) else self.accelerator.device)
