...
class IterativeSFTTrainer(Trainer):
    ...
    def prepare_model_inputs(self, input_ids: torch.Tensor, attention_mask: torch.Tensor, labels: torch.Tensor):
        ...
        if self.is_encoder_decoder:
            input_data = self.data_collator([{"input_ids": ids, "attention_mask": att, "labels": lab} for ids, att, lab in zip(input_ids, attention_mask, labels)]).to(f'npu:{self.model.device}' if isinstance(self.model.device, int) else self.model.device)
            ...
            input_data = self.data_collator([{"input_ids": ids, "attention_mask": att} for ids, att in zip(input_ids, attention_mask)]).to(f'npu:{self.model.device}' if isinstance(self.model.device, int) else self.model.device)
    ...
    def step(
        ...
        if self.state.global_step == 0:
            self.tr_loss = torch.tensor(0.0).to(f'npu:{self.args.device}' if isinstance(self.args.device, int) else self.args.device)
        ...
        def collator(data):
            ...
            for key in data[0]:
                if key in ["input_ids", "attention_mask", "labels"]:
                    return_dict[key] = torch.stack([d[key] for d in data]).to(f'npu:{self.model.device}' if isinstance(self.model.device, int) else self.model.device)
            return return_dict
