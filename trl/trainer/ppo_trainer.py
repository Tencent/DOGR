...
class PPOTrainer(BaseTrainer):
        ...
        **generation_kwargs,
        ...
        for i in range(0, len(query_tensors), batch_size):
            ...
            padded_inputs = self.tokenizer.pad(
                ...
                return_tensors="pt",
            ).to(f'npu:{self.current_device}' if isinstance(self.current_device, int) else self.current_device)
    ...
    def _step_safety_checker(
        ...
        queries = [tensor.to(f'npu:{self.current_device}' if isinstance(self.current_device, int) else self.current_device) for tensor in queries]
        responses = [tensor.to(f'npu:{self.current_device}' if isinstance(self.current_device, int) else self.current_device) for tensor in responses]
        scores = [tensor.to(f'npu:{self.current_device}' if isinstance(self.current_device, int) else self.current_device) for tensor in scores]
        masks = [tensor.to(f'npu:{self.current_device}' if isinstance(self.current_device, int) else self.current_device) for tensor in masks] if masks is not None else None
    ...
    def step(
        ...
        scores = torch.tensor(scores, device=f'npu:{self.current_device}' if isinstance(self.current_device, int) else self.current_device)
        if self.config.use_score_scaling:
            # Score scaling
            scores_mean, scores_std = self.running.update(scores)
            tensor_to_kwargs = dict(dtype=scores.dtype, device=scores.device)
            score_scaling_factor = self.running.std.to(f'npu:{tensor_to_kwargs}' if isinstance(tensor_to_kwargs, int) else tensor_to_kwargs) + torch.finfo(scores.dtype).eps
            if self.config.use_score_norm:
                scores = (scores - self.running.mean.to(f'npu:{tensor_to_kwargs}' if isinstance(tensor_to_kwargs, int) else tensor_to_kwargs)) / score_scaling_factor
            else:
        ...
        if self.config.score_clip is not None:
            ...
            scores = torch.clip(scores.float(), -self.config.score_clip, self.config.score_clip).to(f'npu:{scores_dtype}' if isinstance(scores_dtype, int) else scores_dtype)
        ...
        batch_dict = {
            "queries": queries,
            "responses": responses,
            "logprobs": all_logprobs.to(f'npu:{torch.float32}' if isinstance(torch.float32, int) else torch.float32),
            "values": values.to(f'npu:{torch.float32}' if isinstance(torch.float32, int) else torch.float32),
            "masks": masks,
    ...
    def gather_stats(self, stats):
        ...
        for k, v in stats.items():
            if isinstance(v, torch.Tensor):
                dist.all_reduce(v.to(f'npu:{self.accelerator.device}' if isinstance(self.accelerator.device, int) else self.accelerator.device), dist.ReduceOp.SUM)
    ...
    def prepare_model_inputs(self, queries: torch.Tensor, responses: torch.Tensor):
        if self.is_encoder_decoder:
            input_data = self.data_collator([{"input_ids": q, "attention_mask": torch.ones_like(q)} for q in queries]).to(f'npu:{self.current_device}' if isinstance(self.current_device, int) else self.current_device)

            decoder_inputs = self.data_collator([{"input_ids": r, "attention_mask": torch.ones_like(r)} for r in responses]).to(f'npu:{self.current_device}' if isinstance(self.current_device, int) else self.current_device)

            input_data["decoder_input_ids"] = decoder_inputs["input_ids"]
            input_data["decoder_attention_mask"] = decoder_inputs["attention_mask"]
        else:
            input_ids = [torch.cat([q, r]) for q, r in zip(queries, responses)]
            input_data = self.data_collator([{"input_ids": ids, "attention_mask": torch.ones_like(ids)} for ids in input_ids]).to(f'npu:{self.current_device}' if isinstance(self.current_device, int) else self.current_device)
    ...
    def log_stats(
        ...
        if not isinstance(rewards, torch.Tensor):
            rewards = torch.tensor(rewards).to(f'npu:{self.current_device}' if isinstance(self.current_device, int) else self.current_device)
