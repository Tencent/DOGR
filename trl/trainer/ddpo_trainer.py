...
class DDPOTrainer(BaseTrainer):
    ...
    def __init__(
        ...
        self.sd_pipeline.vae.to(f'npu:{self.accelerator.device}' if isinstance(self.accelerator.device, int) else self.accelerator.device, dtype=inference_dtype)
        self.sd_pipeline.text_encoder.to(f'npu:{self.accelerator.device}' if isinstance(self.accelerator.device, int) else self.accelerator.device, dtype=inference_dtype)
        self.sd_pipeline.unet.to(f'npu:{self.accelerator.device}' if isinstance(self.accelerator.device, int) else self.accelerator.device, dtype=inference_dtype)
        ...
        self.neg_prompt_embed = self.sd_pipeline.text_encoder(
            ...
            ).input_ids.to(f'npu:{self.accelerator.device}' if isinstance(self.accelerator.device, int) else self.accelerator.device)
        )[0]
    ...
    def compute_rewards(self, prompt_image_pairs, is_async=False):
        if not is_async:
            ...
            for images, prompts, prompt_metadata in prompt_image_pairs:
                ...
                rewards.append(
                    (
                        torch.as_tensor(reward, device=f'npu:{self.accelerator.device}' if isinstance(self.accelerator.device, int) else self.accelerator.device),
                        reward_metadata,
                    )
                )
        else:
            rewards = self.executor.map(lambda x: self.reward_fn(*x), prompt_image_pairs)
            rewards = [(torch.as_tensor(reward.result(), device=f'npu:{self.accelerator.device}' if isinstance(self.accelerator.device, int) else self.accelerator.device), reward_metadata.result()) for reward, reward_metadata in rewards]
    ...
    def step(self, epoch: int, global_step: int):
        ...
        samples["advantages"] = torch.as_tensor(advantages).reshape(self.accelerator.num_processes, -1)[self.accelerator.process_index].to(f'npu:{self.accelerator.device}' if isinstance(self.accelerator.device, int) else self.accelerator.device)
        ...
        for inner_epoch in range(self.config.train_num_inner_epochs):
            ...
            perm = torch.randperm(total_batch_size, device=f'npu:{self.accelerator.device}' if isinstance(self.accelerator.device, int) else self.accelerator.device)
            samples = {k: v[perm] for k, v in samples.items()}

            # shuffle along time dimension independently for each sample
            # still trying to understand the code below
            perms = torch.stack([torch.randperm(num_timesteps, device=f'npu:{self.accelerator.device}' if isinstance(self.accelerator.device, int) else self.accelerator.device) for _ in range(total_batch_size)])

            for key in ["timesteps", "latents", "next_latents", "log_probs"]:
                samples[key] = samples[key][
                    torch.arange(total_batch_size, device=f'npu:{self.accelerator.device}' if isinstance(self.accelerator.device, int) else self.accelerator.device)[:, None],
                    perms,
                ]
