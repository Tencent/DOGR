...
def get_global_statistics(accelerator, xs: torch.Tensor, mask=None, device="cpu") -> Tuple[float, float, int]:
    ...
    """
    xs = xs.to(f'npu:{accelerator.device}' if isinstance(accelerator.device, int) else accelerator.device)
    sum_and_count = torch.tensor([xs.sum(), (xs.numel() if mask is None else mask.sum())], device=f'npu:{xs.device}' if isinstance(xs.device, int) else xs.device)
    ...
    return global_mean.to(f'npu:{device}' if isinstance(device, int) else device), global_var.to(f'npu:{device}' if isinstance(device, int) else device), count.to(f'npu:{device}' if isinstance(device, int) else device)
...
def pad_to_length(tensor: torch.Tensor, length: int, pad_value: Union[int, float], dim: int = -1) -> torch.Tensor:
    if tensor.size(dim) >= length:
        ...
        return torch.cat(
            [
                tensor,
                pad_value * torch.ones(*pad_size, dtype=tensor.dtype, device=f'npu:{tensor.device}' if isinstance(tensor.device, int) else tensor.device),
            ],
            dim=dim,
        )
